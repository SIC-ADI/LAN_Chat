...
async def ban_user(target_username: str, reason: str = "No reason"):
    ...
    for ws, info in list(authenticated_clients.items()):
...
async def unban_user(target_username: str):
    if target_username in banned_users:
        ...
        log.info(f"UNBANNED: {target_username}")
        return True
    return False
    ...
    try:
        async for raw in websocket:
            ...
            # ── CHAT MESSAGE ─────────────────────────────────────────
                ...
                info = authenticated_clients.get(websocket)
                if not info:
                    continue
                ...
                if username in banned_users:
                    await send({"type": "error", "text": "You are banned"})
                    continue
                ...
                if text.startswith("/ban "):
                    if role != "admin":
                        await send({"type": "error", "text": "Admin only"})
                        continue
                ...
                if text.startswith("/unban "):
                    ...
                    target = text.split(" ", 1)[1].strip()
                    success = await unban_user(target)

                    if success:
                        await send({"type": "system", "text": f"✅ Unbanned {target}"})
                    else:
                        await send({"type": "error", "text": f"{target} was not banned"})
                    continue
